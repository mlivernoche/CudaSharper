...
__global__ void addArraysKernel(int *result, int *a, int *b, int count) {
...
__global__ void addArraysKernel(float *result, float *a, float *b, int count) {
	int xid = (blockIdx.x * blockDim.x) + threadIdx.x;
	if (xid < count) {
		result[xid] = a[xid] + b[xid];
	}
...
__global__ void addArraysKernel(double *result, double *a, double *b, int count) {
	int xid = (blockIdx.x * blockDim.x) + threadIdx.x;
	if (xid < count) {
		result[xid] = a[xid] + b[xid];
	}
...
__global__ void addArraysKernel(long *result, long *a, long *b, int count) {
	int xid = (blockIdx.x * blockDim.x) + threadIdx.x;
	if (xid < count) {
		result[xid] = a[xid] + b[xid];
	}
...
template<typename T> void addArrays(int device_id, T *result, T *array1, T *array2, const int full_idx) {
	...
	cudaMemcpyAsync(d_a, array1, sizeof(T) * full_idx, cudaMemcpyHostToDevice, stream1);
	cudaMemcpyAsync(d_b, array2, sizeof(T) * full_idx, cudaMemcpyHostToDevice, stream1);
	cudaMemcpyAsync(dResult, result, sizeof(T) * full_idx, cudaMemcpyHostToDevice, stream1);
...
void AddIntArrays(int device_id, int *result, int *array1, int *array2, const int full_idx) {
	addArrays<int>(device_id, result, array1, array2, full_idx);
}

void AddFloatArrays(int device_id, float *result, float *array1, float *array2, const int full_idx) {
	addArrays<float>(device_id, result, array1, array2, full_idx);
}

void AddLongArrays(int device_id, long *result, long *array1, long *array2, const int full_idx) {
	addArrays<long>(device_id, result, array1, array2, full_idx);
}

void AddDoubleArrays(int device_id, double *result, double *array1, double *array2, const int full_idx) {
