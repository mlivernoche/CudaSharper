...
public:
	...
	__inline__ void force_allocate() {
		if (this->ptr == NULL) {
			cudaSetDevice(this->cuda_device_id);
			this->ptr = std::make_unique<thrust::device_vector<T>>(this->heap_size);
		}
	}
...
private:
	std::unique_ptr<thrust::device_vector<T>> ptr;
...
public:
	...
	__inline__ void force_allocate() {
		if (this->device_ptr_u32 == NULL) {
			device_ptr_u32 = std::make_unique<cuda_device_ptr<int32_t>>(this->cuda_device_id, this->heap_size);
			device_ptr_u32->force_allocate();
		}

		if (this->device_ptr_u64 == NULL) {
			device_ptr_u64 = std::make_unique<cuda_device_ptr<int64_t>>(this->cuda_device_id, this->heap_size);
			device_ptr_u64->force_allocate();
		}

		if (this->device_ptr_f32 == NULL) {
			device_ptr_f32 = std::make_unique<cuda_device_ptr<float>>(this->cuda_device_id, this->heap_size);
			device_ptr_f32->force_allocate();
		}

		if (this->device_ptr_f64 == NULL) {
			device_ptr_f64 = std::make_unique<cuda_device_ptr<double>>(this->cuda_device_id, this->heap_size);
...
private:
	int32_t cuda_device_id;
	int32_t heap_size;
	std::unique_ptr<cuda_device_ptr<int32_t>> device_ptr_u32;
	std::unique_ptr<cuda_device_ptr<int64_t>> device_ptr_u64;
	std::unique_ptr<cuda_device_ptr<float>> device_ptr_f32;
	std::unique_ptr<cuda_device_ptr<double>> device_ptr_f64;
};
