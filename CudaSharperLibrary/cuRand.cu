...
__global__ void uniform_rand_kernel(curandState_t *states, float *numbers, unsigned int count, unsigned int maximum) {
	...
	// unsigned int sharedMem = (sizeof(int) * 2) + ((sizeof(curandState_t) + sizeof(int) + sizeof(int)) * CURAND_NUM_OF_THREADS);
	...
	int *endShared = (int*)&startShared[CURAND_NUM_OF_THREADS];
...
__global__ void log_normal_rand_kernel(curandState_t *states, float *numbers, unsigned int count, unsigned int maximum, float mean, float stddev) {
	...
	// unsigned int sharedMem = (sizeof(int) * 2) + (sizeof(float) * 2) + ((sizeof(curandState_t) + sizeof(int) + sizeof(int)) * CURAND_NUM_OF_THREADS);
	int *maximumShared = (int*)&smem[0];
	int *countShared = (int*)&maximumShared[1];
	...
	for (int n = startShared[threadIdx.x]; n < endShared[threadIdx.x]; n++) {
		if (n < maximumShared[0]) {
			numbers[n] = curand_log_normal(&curandStateShared[threadIdx.x], meanShared[0], stdDevShared[0]);
		}
	}
